...
const version = "1.3-cyl";  // Fork: adds cylindrical (unrolled IP) detector geometry.
...
let detGeometry = 'flat';   // 'flat' (planar detector) or 'cylinder' (unrolled cylindrical IP, radius = Lsd)
...
window.addEventListener('load', () => {
    ...
    const detGeomFlat = document.getElementById('detGeom_flat');
    if(detGeomFlat!=null){
        detGeomFlat.addEventListener('change', (evt) => {
            draw_DetMap();
        });
    }
    const detGeomCyl = document.getElementById('detGeom_cyl');
    if(detGeomCyl!=null){
        detGeomCyl.addEventListener('change', (evt) => {
            draw_DetMap();
        });
    }
...
function draw_DetMap(){
    ...
    let detGeomCylEl = document.getElementById('detGeom_cyl');
    detGeometry = (detGeomCylEl!=null && detGeomCylEl.checked==true) ? 'cylinder' : 'flat';
...
function drawBraggReflection(context1,H1,K1,L1,isTargetHKL1,showHKL1){
    ...
    if(Ghkl[0]>=0.0){
        ...
        if(lambda>lambda_min && G_sq > 0 && G_sq < Qmax**2.0){   // lambda_min=2PI/sqrt(Ei_max/2.072), the case that H=K=L=0 is avoided by the condition of  G_sq > 0.
            ...
            let PosX;
            let PosY;
            let onDetector=false;

            if(detGeometry=='cylinder'){
                //Cylindrical imaging plate (unrolled). The sample sits on the cylinder axis with radius R=Lsd,
                //and the pinhole is at -x. Both backscattering and part of the forward-scattering reach the plate.
                //Horizontal: arc length u = R * delta, delta = azimuth from the pinhole (-x), +y side positive.
                //Vertical:   v = R * kfz / sqrt(kfx^2+kfy^2)  (cylinder axis, up = negative screen Y).
                let rho=Math.sqrt(kf[0]**2.0+kf[1]**2.0);
                if(rho>1e-12){
                    let delta=Math.atan2(kf[1],-kf[0]);
                    PosX=Lsd*delta*mm2pixel+X0+X0_ofst;
                    PosY=-Lsd*kf[2]/rho*mm2pixel+Y0+Y0_ofst;
                    onDetector=true;
                }
            }
            else if(kf[0]<0){    //Flat detector, backscattering condition: the scattered X-ray must go to -x direction, namely opposite to the incident x-ray direction.
                PosX=-kf[1]/kf[0]*Lsd*mm2pixel+X0+X0_ofst;
                PosY=kf[2]/kf[0]*Lsd*mm2pixel+Y0+Y0_ofst;
                onDetector=true;
            }
